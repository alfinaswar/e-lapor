...
@section('content')
    ...
    <div class="row mb-3">
        <div class="col text-end">
            <a class="btn btn-primary" href="{{ route('laporan-dinas.create') }}">Tambah Laporan Dinas Baru</a>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            $('body').on('click', '.btn-delete', function() {
                ...
                Swal.fire({
                    title: 'Hapus Data?',
                    text: "Apakah Anda yakin ingin menghapus laporan dinas ini?",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ route('laporan-dinas.destroy', ':id') }}'.replace(
                                ':id', id),
                            type: 'DELETE',
                            data: {
                                _token: '{{ csrf_token() }}'
                            ...
                            success: function(response) {
                                if (response.status === 200) {
                                    ...
                                    $('#laporanDinasTable').DataTable().ajax.reload();
                                } else {
            ...
            function loadDataTable() {
                $('#laporanDinasTable').DataTable({
                    responsive: true,
                    serverSide: true,
                    processing: true,
                    bDestroy: true,
                    ajax: "{{ route('laporan-dinas.index') }}",
                    language: {
